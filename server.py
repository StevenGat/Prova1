...
FARMACI = [
    {"id":"1","nome":"Dailyvit B12 flaconcini",     "qty":4,
     "url":"https://www.trovaprezzi.it/prezzo_integratori-coadiuvanti_984984308.aspx"},
    {"id":"2","nome":"Collirio Plus 10ml",           "qty":5,
     "url":"https://www.trovaprezzi.it/prezzo_prodotti-salute_984158865.aspx"},
    {"id":"3","nome":"Alfa Collirio Idratante 10ml", "qty":5,
     "url":"https://www.trovaprezzi.it/prezzo_prodotti-salute_987055872.aspx"},
    {"id":"4","nome":"Norsan 120 Arktis",            "qty":5,
     "url":"https://www.trovaprezzi.it/prezzo_integratori-coadiuvanti_981499054.aspx"},
    {"id":"5","nome":"Dailyvit Senior",              "qty":5,
     "url":"https://www.trovaprezzi.it/prezzo_integratori-coadiuvanti_930629934.aspx"},
    {"id":"6","nome":"Essaven Gel 80g",              "qty":1,
     "url":"https://www.trovaprezzi.it/prezzo_farmaci-da-banco_036193023.aspx"},
    {"id":"7","nome":"Aximagnesio 20 bustine",       "qty":2,
     "url":"https://www.trovaprezzi.it/prezzo_integratori-coadiuvanti_972069633.aspx"},
    {"id":"8","nome":"Fluimucil 600mg bustine",      "qty":1,
     "url":"https://www.trovaprezzi.it/prezzo_farmaci-da-banco_034936169.aspx"},
    {"id":"9","nome":"Apropos Vita+ Mag+Pot",        "qty":2,
     "url":"https://www.trovaprezzi.it/prezzo_integratori-coadiuvanti_979043116.aspx"},
]
    ...
    for page_url in [url, url + "?page=2"]:
        try:
            ...
            for item in items:
                ...
                offerte.append({
                    "shop"  : s.get_text(strip=True),
                    "prezzo": prezzo, "sped": sped,
                    "totale": round(prezzo + sped, 2),
    ...
    for f in FARMACI:
        for o in get_offerte(f["id"]):
            ...
            farmacie[o["shop"]]["dettaglio"].append({
                "farmaco":f["nome"],"qty":f["qty"],
                "prezzo":o["prezzo"],"sped":o["sped"],
                "totale_qty":round(o["totale"]*f["qty"],2)})
    result = sorted([
        {"shop":k,"trovati":v["trovati"],"totale":round(v["totale"],2),"dettaglio":v["dettaglio"]}
        for k,v in farmacie.items()],
        key=lambda x:(-x["trovati"],x["totale"]))
